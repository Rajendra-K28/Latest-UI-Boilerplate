...
import type { AssetRecord, AssetVulnerability } from '../ScanComponentsView';
import { AssetIcon } from '../ScanComponentsView';

type VulnTabKey = 'all' | 'critical' | 'high' | 'medium' | 'low';

type AssetVulnerabilitiesDrawerProps = {
  asset: AssetRecord;
  vulnerabilities: AssetVulnerability[];
  activeTab: VulnTabKey;
  onTabChange: (tab: VulnTabKey) => void;
  onClose: () => void;
};

export function AssetVulnerabilitiesDrawer({
  asset,
  vulnerabilities,
  activeTab,
  onTabChange,
  onClose,
}: AssetVulnerabilitiesDrawerProps) {
  const allVulns = vulnerabilities;
  const filtered =
    activeTab === 'all'
      ? allVulns
      : allVulns.filter((v) => v.severity === activeTab);
  ...
  const tabLabels: { key: VulnTabKey; label: string }[] = [
    { key: 'all', label: 'All' },
  ...
  return (
    <SideDrawer
      ...
      onClose={onClose}
      title={asset.name}
      subtitle={`${asset.ipAddress} · ${asset.typeLabel} · Last scanned ${asset.lastScanned}`}
